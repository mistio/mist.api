...
def _query_cores(start, owner_id=''):
    ...
    return [
        {
            ...
            'values': values
        } for owner, values in data.iteritems()
    ]
...
def get_usage(owner_id='', full_days=6):
    ...
    data = _merge_series(_query_cores(start, owner_id),
                         _query_checks(start, end, owner_id),
                         _query_datapoints(start, end, owner_id))
    return [
        {
            'date': d,
