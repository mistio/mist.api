...
def _populate_clusters():
    ...
    for key, value in list(globals().items()):
        if not key.startswith("_") and key.endswith("Cluster") and key != "Cluster":
            if issubclass(value, Cluster) and value is not Cluster:
                CLUSTERS[value.provider] = value
                for provider_alias in api_config.PROVIDERS[value.provider]["aliases"]:
...
class Cluster(OwnershipMixin, me.Document):
    ...
    location = me.ReferenceField(
        "CloudLocation", required=False, reverse_delete_rule=me.DENY
    )
    name = me.StringField(required=True)
    ...
    extra = MistDictField()
    state = me.StringField(
        default="unknown", choices=api_config.CLUSTER_STATES)
    ...
    def tags(self):
        return {
            ...
            for tag in Tag.objects(
                owner=self.owner, resource_id=self.id, resource_type="cluster"
            ).only("key", "value")
        }
    ...
    def __str__(self):
        return "%s cluster %s (%s) of %s" % (type(self), self.name, self.id, self.owner)
