...
def get_stories(story_type='', owner_id='', user_id='',
    ...
    field is a list of lists in the form of:
        ...
        (opens|closes|updates, job|shell|session|request, story_id)

    which intends to describe how a certain log affects a story. Logs may open,
    close, or update stories of type job, shell, session, or request. Each log
...
def process_stories(buckets, type=None, pending=None, callback=None):
    ...
    for bucket in buckets:
        ...
        for doc in bucket['top_logs']['hits']['hits']:
            ...
            for act, _, sid in body.pop('stories', []):
...
def associate_stories(event):
    ...
    try:
        ...
        job = None
        log.warn('Failed to extract %s from kwargs: %s', job, exc)

    # Decide whether the event tends to start, update, or close a story.
    action = 'updates'
    if event['error']:
        action = 'closes'
...
def close_story(owner_id, story_id):
    ...
    body = doc['_source']
    # Get story_id and proper story_type.
    ...
    for story in body['stories']:
        if tuple(story) == ('closes', doc_type, story_id):
            log.error('Story %s already closed', story_id)
            break
    else:
        body['stories'].append(('closes', doc_type, story_id))
