    ...
    def create_machine(self, plan):
        ...
        if not sec_group:
            ...
            for sg in sec_groups:
                ...
                raise BadRequestError("Security group not found: %s"
                                      % sec_group)
        ...
        if plan.get('subnet_id'):
            from mist.api.networks.models import Subnet
            try:
                subnet = Subnet.objects.get(id=plan['subnet_id'])
            ...
            subnets = self.connection.ex_list_subnets()
            ...
            if not sec_group:
                groups = self.connection.ex_list_security_groups()
                for group in groups:
        ...
        for volume in plan.get('volumes', []):
            if volume.get('volume_id'):
                try:
                    ...
                    libcloud_disks = self.connection.list_volumes()
