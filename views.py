...
def create_machine(request):
    ...
    kwargs = {'script_id': script_id, 'script_params': script_params,
              ...
              'hostname': hostname, 'plugins': plugins, 'disk_size': disk_size,
...
def machine_actions(request):
    ...
    if action in ('start', 'stop', 'reboot', 'destroy', 'resize', 'rename', 'vnc'):
        if action == 'start':
            ...
            methods.vnc_machine(user, backend_id, machine_id, name)
        # return OK
        return methods.list_machines(user, backend_id)
